...
class ProxConverter(BaseConverter):
    ...
    def convert(self, dataset_path: str, out_path: str) -> dict:
            ...
            for img_name in os.listdir(img_dir):
                ...
                with open(keypoints_file) as f:
                    ...
                    keypoints2d[keypoints2d[:, 2] > 0.15,
                                2] = 1  # set confidence (0.2 for now)
                ...
                if not os.path.exists(smpl_file):
                    ...
                    with open(smpl_file, 'rb') as f:
                        ...
                        smplx['left_hand_pose'].append(ann['left_hand_pose'])
                        smplx['right_hand_pose'].append(ann['right_hand_pose'])
        ...
        # smplx
        smplx['left_hand_pose'] = np.array(smplx['left_hand_pose']).reshape(
            (-1, 4, 3))
        smplx['right_hand_pose'] = np.array(smplx['right_hand_pose']).reshape(
            (-1, 4, 3))
        ...
        out_file = os.path.join(out_path, 'prox_train.npz')
