...
class ThreeDOH50kConverter(BaseModeConverter):
    """CrowdPose dataset
    ...
    def __init__(self, modes: List = []) -> None:
        super(ThreeDOH50kConverter, self).__init__(modes)
    ...
    def convert_by_mode(self, dataset_path: str, out_path: str,
        ...
        for fid in tqdm(json_data.keys()):
            ...
            # bbox = np.array(annot['bbox']).reshape(-1)
            ...
            # scale and center
            bbox_xyxy = np.array(annot['bbox']).reshape(
                -1)  # 2x2 - check format
